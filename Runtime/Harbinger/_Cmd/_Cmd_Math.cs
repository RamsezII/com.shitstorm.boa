            ...
            cmd_math_ = AddContract(new("math",
                args: static exe =>
                {
                    if (exe.reader.TryReadArgument(out string operator_name, out exe.error))
                        ...
                        else if (exe.harbinger.TryParseExpression(exe.reader, exe.scope, true, out var expr1, out exe.error))
                            if (exe.harbinger.TryParseExpression(exe.reader, exe.scope, true, out var expr2, out exe.error))
                            {
                                exe.args.Add(code);
                                exe.args.Add(expr1);
                                exe.args.Add(expr2);
