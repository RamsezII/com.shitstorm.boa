namespace _BOA_
{
    ...
    {
        internal bool TryParseAssignation(in BoaReader reader, in Executor caller, out ExpressionExecutor assignation, out string error)
        {
            assignation = null;
            int read_old = reader.read_i;

            if (reader.TryReadArgument(out string varname, out error, as_function_argument: false))
                if (caller.TryGetVariable(varname, out var variable))
                    if (reader.TryReadString_matches_out(out string op_name, true, skippables: BoaReader._empties_, stoppers: " \n\r{}(),;'\"", "=", "+=", "-=", "*=", "/="))
                    {
                        ...
                        if (TryParseExpression(reader, caller, false, out var expr, out error))
                        {
