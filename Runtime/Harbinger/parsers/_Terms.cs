namespace _BOA_
{
    ...
    {
        internal bool TryParseTerm(in BoaReader reader, in Executor caller, out ExpressionExecutor term, out string error)
        {
            term = null;

            if (!TryParseFactor(reader, caller, out var factor1, out error))
                return false;
            ...
            if (reader.TryReadChar_match_out(out char op_char, true, "*/%"))
            {
                ...
                if (TryParseFactor(reader, caller, out var factor2, out error))
                {
